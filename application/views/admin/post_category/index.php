<div class="clearfix" style="margin-bottom: 10px;">
	<div class="title-admin" style="float: left;">
		<h3 style="margin-top: 6px; font-size: 18px;">QUẢN LÝ DANH MỤC BÀI VIẾT</h3>
	</div>
	<!-- <div class="action-admin" style="float: right;">
		<a class="btn btn-success" href="<?php echo base_url() ?>admin/postCategory/add">Thêm mới</a>
	</div> -->
</div>
			...
			<?php foreach ($post_categories as $item) { ?>
				<tr data-id="<?php echo $item['id'] ?>">
					...
					<td style="vertical-align: middle;">
						<a href="<?php echo base_url() ?>admin/postCategory/edit/<?php echo $item['id'] ?>"><?php echo $item['name'] ?></a>
					</td>
					<td class="text-center" style="vertical-align: middle;">
					...
					<!-- <td class="text-center" style="vertical-align: middle;">
						...
						<input type="checkbox" <?php if ($item['is_on_home'] === 1) echo 'checked' ?> data-id="<?php echo $item['id'] ?>" onclick="updateIsOnHome(this)" />
					</td> -->
					<!-- <td class="text-center" style="vertical-align: middle;">
					...
					<td class="text-center" style="vertical-align: middle;">
						<a class="btn btn-blue btn-sm" href="<?php echo base_url() ?>admin/postCategory/edit/<?php echo $item['id'] ?>">Cập nhật</a>
						<?php if (!in_array($item['name_unsigned'], array('bao-gia', 'du-an', 'tin-tuc'))) { ?>
...
<script type="text/javascript">
	...
	function updateDisplayOrder(el) {
		...
		axios({
				method: 'POST',
				url: '<?php echo base_url() ?>admin/postCategory/update_display_order',
				data: {
					id: id,
	...
	function updateIsHot(el) {
		...
		axios({
				method: 'POST',
				url: '<?php echo base_url() ?>admin/postCategory/update_is_hot',
				data: {
					id: $(el).data('id'),
					is_hot: is_hot
				}
	...
	function updateIsOnHome(el) {
		const id = $(el).data('id');
		const is_on_home = $(el).prop("checked") ? 1 : 0
		axios({
				method: 'POST',
				url: '<?php echo base_url() ?>admin/postCategory/update_is_on_home',
				data: {
					id: $(el).data('id'),
	...
	function updateStatus(el) {
		...
		axios({
				method: 'POST',
				url: '<?php echo base_url() ?>admin/postCategory/update_status',
				data: {
					id: $(el).data('id'),
					is_enable: is_enable
				}
	...
	function deleteCategory(el) {
		...
		axios({
				method: 'POST',
				url: '<?php echo base_url() ?>admin/postCategory/delete',
				data: {
					ids: selectedIds
				}
			})
