				...
				<div class="panel-body">
					<form id="form-main" role="form" class="form-horizontal form-groups-bordered">
						<div class="row form-group">
							...
							<div class="col-sm-9">
								<input type="text" class="form-control" id="name" name="name" <?php echo in_array($category['name_unsigned'], array('san-pham', 'bao-gia', 'du-an', 'tin-tuc')) ? 'disabled' : '' ?> v-model="category.name">
							</div>
						</div>
							...
							<div class="col-sm-12">
								<div class="fileinput fileinput-new" data-provides="fileinput">
									<div class="fileinput-new thumbnail" style="width: 350px; height: <?php echo (int)(350 * POST_CATEGORY_IMAGE_HEIGHT / POST_CATEGORY_IMAGE_WIDTH) ?>px;" data-trigger="fileinput">
										<img src="<?php echo base_url() . POST_UPLOAD_PATH . $category['avatar'] ?>" alt="avatar">
									</div>
...
<script>
	var app = new Vue({
		...
		methods: {
			initData() {
				const data = <?php echo json_encode($category) ?>;
				this.category.id = data.id;
				this.category.name = data.name;
				this.category.parent_id = data.parent_id;
				this.category.description = data.description;
				this.category.display_order = data.display_order;
				this.category.is_enable = data.is_enable === 1 ? true : false;
				this.category.is_on_home = data.is_on_home === 1 ? true : false;
				this.category.is_hot = data.is_hot === 1 ? true : false;
				this.category.meta_keyword = data.meta_keyword;
				this.category.meta_description = data.meta_description;
				// this.category.avatar = data.avatar;
			},
			save() {
				...
				if (validator) {
					...
					axios({
							method: 'POST',
							url: '<?php echo base_url() ?>admin/postCategory/update',
							headers: {
