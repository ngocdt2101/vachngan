					...
					<form id="form-main" role="form" class="form-horizontal form-groups-bordered">
							...
							<div class="col-sm-12">
								<div class="fileinput fileinput-new" data-provides="fileinput">
									<div class="fileinput-new thumbnail" style="width: 600px; height: 450px;" data-trigger="fileinput">
										<img src="<?php echo base_url() . POST_UPLOAD_PATH . $post['avatar'] ?>" alt="avatar">
									</div>
						...
						<div class="row form-group">
							<div class="col-xs-12" style="display: flex; justify-content: center;">
								<a class="btn btn-default btn-sm" href="<?php echo base_url() ?>admin/quotation3" style="margin-right: 10px; font-size: 12px;">Trở về</a>
...
<script>
	var app = new Vue({
		...
		methods: {
			initData() {
				let data = <?php echo json_encode($post) ?>;
				if (data) {
					this.post.id = data.id;
					this.post.name = data.name;
					this.post.name_unsigned = data.name_unsigned;
					this.post.description = data.description;
					this.post.content = data.content;
					this.post.category_id = data.category_id;
					this.post.is_enable = data.is_enable === 1 ? true : false;
					this.post.is_on_home = data.is_on_home === 1 ? true : false;
					this.post.is_hot = data.is_hot === 1 ? true : false;
					this.post.display_order = data.display_order;
					this.post.meta_keyword = data.meta_keyword;
					this.post.meta_description = data.meta_description;
					this.post.view_count = data.view_count;
					this.post.avatar = data.avatar;
				}
			},
			save() {
				...
				axios({
						method: 'POST',
						url: '<?php echo base_url() ?>admin/quotation3/update',
						headers: {
					...
					.then((response) => {
						toastr.success("Cập nhật báo giá thành công", "Thành công !");
						window.location.replace("<?php echo base_url() ?>admin/quotation3");
					})
					.catch((response) => {
